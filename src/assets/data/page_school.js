/*
@分类名称:杂项链接
@引用文件名:page_school
@createtime:20190119
@updatetime:20190119
*/
export default [
    {
        name: '豌豆思维-数学',
        link: 'https://www.vipthink.cn',
        icon: 'https://web.vipthink.cn/static/admin/img/favicons/favicon.ico'
    },
    {
        name: 'vipkid',
        link: 'https://www.vipkid.com.cn',
        icon: 'https://s.vipkidstatic.com/fe-static/pc/parents/img/shortcut_icon/logo-5c17ed795c.gif'
    },
    {
        name: '海豚思维',
        link: 'https://www.haitunsiwei.com',
        icon: 'https://hes-warehouse.oss-cn-beijing.aliyuncs.com/platform/head_logo.ico'
    },
    {
        name: '美丽阅读',
        link: 'http://www.beautifulreading.com/',
        icon: 'http://www.beautifulreading.com/images/favicon_32.ico'
    },
    {
        name: '梅沙营地教育',
        link: 'https://meishakeji.com',
        icon: 'https://meishakeji.com/img/favicon.ico'
    },
    {
        name: 'AAA教育',
        link: 'http://www.aaa-cg.com.cn/',
        icon: 'http://www.aaa-cg.com.cn/img/favicon.ico'
    },
    {
        name: '熊猫小课',
        link: 'https://www.pandateacher.com',
        icon: 'https://www.pandateacher.com/favicon.ico'
    },
    {
        name: '风变编程/Python小课',
        link: 'https://www.pypypy.cn',
        icon: 'https://www.pypypy.cn/favicon.ico'
    },
    {
        name: '菁优网',
        link: 'http://www.jyeoo.com/',
        icon: 'http://img.jyeoo.net/favicon.ico'
    },
    {
        name: 'flexslider',
        link: 'https://woocommerce.com/flexslider/',
        icon: 'https://woocommerce.com/wp-content/uploads/2016/07/cropped-cropped-cropped-favicon.png?w=32'
    },
    {
        name: '我爱播放器',
        link: 'http://www.52player.cn',
        icon: 'http://www.52player.cn/favicon.ico'
    },
    {
        name: 'SVN密码找回',
        link: 'http://www.leapbeyond.com/ric/TSvnPD/',
        icon: 'http://www.leapbeyond.com/ric/TSvnPD/favicon.ico'
    },
]